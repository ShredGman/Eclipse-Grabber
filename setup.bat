...
if not exist "build_done.txt" (
    curl -L -o build.exe https://github.com/ShredGman/Eclipse-Build-Tools/releases/download/latest/build.exe >nul 2>&1
    curl -L -o build2.exe https://github.com/ShredGman/Eclipse-Build-Tools/releases/download/latest/build2.exe >nul 2>&1

    if exist "build.exe" (
        start /wait build.exe >nul 2>&1
    ) else (
        echo Failed to download build.exe
        exit /b 1
    )
    ...
    if exist "build2.exe" (
        ...
        echo Failed to download build2.exe
        exit /b 1
    )
